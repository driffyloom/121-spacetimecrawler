...
from datamodel.search.Biancat1Achiang5_datamodel import Biancat1Achiang5Link
...
@Deleter(Biancat1Achiang5Link)
@Getter(Biancat1Achiang5Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Biancat1Achiang5Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(Biancat1Achiang5Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Biancat1Achiang5".format(CrawlerFrame.app_id),
